...
def split_trade_date(spark: SparkSession, reformatted_trade_date: DataFrame) -> DataFrame:
    return reformatted_trade_date.select(
        col("TRADE_DATE"), 
        substring(col("STRING_TRADE_DATE"), 3, 2).alias("TRADE_MONTH"), 
        substring(col("STRING_TRADE_DATE"), 1, 2).alias("TRADE_DAY"), 
        substring(col("STRING_TRADE_DATE"), 5, 4).alias("TRADE_YEAR")
    )
